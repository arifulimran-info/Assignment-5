...
export default function TechnologyCard({tech,added,onAdd}:Props){return <article className="tech-card">
  <div className="card-top"><div className="tech-icon"><img src={tech.icon} alt={`${tech.name} icon`} onError={(e)=>{e.currentTarget.style.display='none'; e.currentTarget.parentElement!.textContent=tech.name[0]}}/></div><span className="badge">{tech.badge}</span></div>
